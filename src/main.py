...
def parse_yearly_data(activities: list[SummaryActivity]) -> Tuple[int, int, int, list[int]]:
  now = datetime.now()
  jan_first = datetime(year=now.year, month=1, day=1)
  ...
  return total_activities, round(total_mileage, 2), round(total_mileage / weeks_ytd, 2), np.round(mileage_per_month, 2)
